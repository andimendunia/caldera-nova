<x-layout-kpi title="{{ $title }}" header="{{ $header }}" prev="{!! $prev !!}" nav="{{ $nav }}" navs="{{ $navs }}">
    ...
    <div class="max-w-xl lg:max-w-2xl mx-auto px-4 py-16">        
        ...
        <p class="mt-4 mb-12 text-lg text-neutral-500">{{ __('Lapor KPI bulanan.') }}</p>
        ...
        <ul class="space-y-4 text-left text-neutral-500 dark:text-neutral-400">
            ...
            <li class="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <span><span class="font-semibold text-neutral-900 dark:text-white">{{ __('Penyerahan') }}</span>{{ ' ' . __('untuk melaporkan pencapaian KPI bulanan beserta data pendukung.') }}</span>
            </li>
            <li class="flex items-center space-x-3 rtl:space-x-reverse">
                <i class="fa fa-ellipsis-h fa-fw me-2"></i>
                <span><span class="font-semibold text-neutral-900 dark:text-white">{{ __('Administrasi') }}</span>{{ ' ' . __('untuk membuat item KPI, kelola wewenang, dan lainnya.') }}</span>
            </li>
        </ul>
    </div>
    @endswitch
